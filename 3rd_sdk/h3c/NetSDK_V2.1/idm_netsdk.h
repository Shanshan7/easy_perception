...
#define IDM_ERROR_SN_EXIST                  (IDM_ERROR_DEFINED + 32)        /* SN已存在 */
...
#define EXCEPTION_KEEPALIVE                 0     /*设备连接异常(设置了断线重连,将会恢复连接)*/
#define EXCEPTION_REALPLAY                  1
#define EXCEPTION_PLAYBACK                  2
#define EXCEPTION_ALARM                     3     /*告警连接异常(设置了断线重连,将会恢复告警)*/
#define EXCEPTION_VOICE                     4
#define EXCEPTION_RECONNECT                 5     /*设备重连成功*/
#define EXCEPTION_CHN_OFFLINE               6
#define EXCEPTION_RECONNECT_FAILED          7     /*设备重连失败(设置了断线重连, 恢复连接时设备报错)*/
#define EXCEPTION_ALARM_RECONNECT           8     /*告警恢复成功*/
...
#define PTZ_3D_ZOOM                         32	//3D定位 IDM_DEV_PTZ_3D_ZOOM_INFO_S
...
#define PTZ_ASSIST_FOCUS					74 //辅助聚焦             IDM_DEV_ASSIST_FOCUS_INFO_S
...
#define CONFIG_SYSTEM_TIME_V2               0x00000004
#define CONFIG_SYSTEM_RUN_MODE				0x00000005  //工作模式(设置工作模式,会重启设备)
#define CONFIG_IMAGE_CALIBRATION			0x00000006  //开启标定参数
#define CONFIG_IMAGE_STITCH_DISTANCE		0x00000007  //标定距离
...
#define CONFIG_PTZ_PRESET_LIST              0x00000504
#define CONFIG_PTZ_PT_ZOOM                  0x00000510 //获取PT、Zoom、聚焦标志
...
/* ITS路口机交通配置 */
#define CONFIG_ITS_CHECK_POINT_TRIGGER_MODE 0x00000901  /*卡口触发模式配置*/
#define CONFIG_ITS_CHECK_POINT_VIDEO		0x00000902  /*视频卡口配置*/
#define CONFIG_ITS_CHECK_POINT_RADAR		0x00000903  /*雷达卡口配置*/
#define CONFIG_ITS_TRAFFIC_EPOLICE			0x00000904  /*视频电警配置*/
#define CONFIG_ITS_TRAFFIC_FLOWRATE			0x00000905  /*车流量统计配置*/
#define CONFIG_ITS_TRAFFIC_ROAD_EVENT		0x00000906  /*道路事件配置*/
#define CONFIG_ITS_LED_CORRECT				0x00000907  /*信号灯校正配置*/
...
typedef struct tagIDM_DEV_VIDEO_ENCODE_PARAM
{
	...
	USHORT	usImageWidth;							/* 视频图像宽度 非必填,当usResolution=99时必须要填写实际分辨率*/
	USHORT	usImageHeight;							/* 视频图像高度 非必填,当usResolution=99时必须要填写实际分辨率*/
	UCHAR	ucThirdStreamEnable;					/* 第三码流是否开启,非必填 1开启,0关闭,默认0, 适用A3工程商款*/
	UCHAR	ucBncOutputEnable;						/* BNC接口模拟输出开关, 非必填. 1开启,0关闭, 默认0 */
    UCHAR   aucRes[10];
...
typedef struct tagIDM_DEV_REQUEST_PARAM
{
    ...
    ULONG   ulTimeout;                              /* 超时时间：单位：ms，默认10s */
...
typedef struct tagIDM_DEV_PREST_INFOS
{
	USHORT usPageNO;									/*[in] 分页号[1-N] */
	USHORT usPageSize;								/*[in] 每页数据条数 */
	IDM_DEV_PRESET_NAME_S *pstPrests;				/*[out] 预置点结构体数组,用于get时,内部申请并返回*/
	USHORT usCount;									/*[out] pstPrests中的预置点个数*/
	USHORT usTotalCount;							/*[out] 所有预置点总数*/
	UCHAR aucRes[128];								/*保留数据*/
}IDM_DEV_PREST_INFOS_S;
...
typedef VOID(CALLBACK *IDM_DEV_Exception_Callback_PF)(
    LONG    lUserID,  /* 来自哪个设备的userid */
    LONG    lHandle,  /* 由具体类型决定其含义 比如通道离线异常标识离线的通道号 */
    ULONG   ulType,   /* 事件类型 例 EXCEPTION_KEEPALIVE  */
    VOID    *pUserData
    );
...
typedef struct tagIDM_DEV_USER_LOGIN_INFO
{
    CHAR    szDeviceIP[IDM_DEVICE_IP_MAX_LEN];      /* 设备IP地址 */
    USHORT  usPort;                                 /* 设备端口号 */
	UCHAR   ucRes1;                                 /* 内部使用  请置0*/
	UCHAR   ucRes2;                                 /* 内部使用  请置0*/       
    CHAR    szUsername[IDM_USERNAME_MAX_LEN];       /* 登录用户名 */
    CHAR    szPassword[IDM_PASSWORD_MAX_LEN];       /* 登录密码 */
...
typedef VOID(CALLBACK *IDM_DEV_RealPlay_Callback_PF) (
    ...
    ULONG   ulDataType, //0视频帧,1音频帧,2其它数据
    UCHAR   *pucBuffer,//私有封装格式数据
    ULONG   ulBufferSize,
    VOID    *pUserData
    );
...
typedef struct tagIDM_DEV_PREVIEW_INFO
{
	...
	UCHAR	ucStreamMode;							/* 流模式, 0:音频复合流, 1:纯视频流 2:纯音频流*/
    UCHAR   aucRes[254];
...
typedef struct tagIDM_DEV_PACKET_INFO
{
    ...
    ULONG   ulPacketType;                           /* 数据类型: 帧类型，0xF1-视频I帧，0xF2-视频P帧，0xF3-MJPEG图片帧，0xF4-辅助帧，0xF5-音频帧，0xF6-视频B帧 */
    ULONG   ulEncodeType;                           /* 编码类型: 视频帧时: (只在I帧时才有编码格式) 1: MJPEG  2: H.264  3: H.265  4:  MPEG4
														音频帧时: 1: ADPCM 2:G.722 3:G.711U 4:G.711A 5:G.726 6:AAC 7:MP2L2 8:PCM 9:G.722.1*/
    ULONG   ulPacketMode;                           /* 打包方式  0: 默认  */
    ULONG   ulTimeStamp;                            /* 时间戳低位  时间戳的低位4个字节  */
    ULONG   ulTimeStampHight;                       /* 时间戳高位  时间戳的高位4个字节  可以定义 8 字节的 时间戳 取  高位左移32位和低位相加 
															比如 unsigned long long  timestamp =  hight << 32  + low; */
    ULONG   ulFrameRate;                            /* 帧率 */
    USHORT  usWidth;                                /* 宽度 */
    USHORT  usHeight;                               /* 高度 */
    ULONG   ulBufferSize;                           /* 数据大小 */
    UCHAR   *pucBuffer;                             /* 数据缓冲指针 数据是H264/H265裸流帧数据 */
    UCHAR   aucRes[64];
...
typedef struct tagIDM_SNAP_PIC_DOWNLOAD_PROGRESS
{
	...
	UINT     uiNameLength;                                      /* 下载失败文件名称(pNameFail)长度 */
	CHAR     *pNamesFail;                                       /* 下载失败图片名称 以,隔开(需要使用IDM_FREE释放) */
    UINT     uiTotalPicNum;                                     /* 图片总张数 */
    CHAR     cDownlaodStatus;                                   /* 0-下载中 1-下载完成 2-无数据(查询图片数为0/下载过程中设备SD卡拔出) */
	UCHAR    aucRes[3];
...
{
    UINT  uFileTime;                /*录制文件最大切包时长 大于0有效 单位秒*/
    UINT  uFileSize;                /*录制文件最大切包大小 大于0有效 单位字节*/
    CHAR  szFileName[256];          /*录制文件名称 包含路径和名称(如:c:/test/test.mp4)，
                                    切文件时文件名在末尾追加_part1,_part2,...,(c:/test/test_part1.mp4)
                                    满足下列其中一个条件将会切文件继续录制:
                                    1.文件大小达到最大切包大小(默认2G)且是视频关键帧时，
                                    2.文件时长达到最大切包时长(默认1小时)且是视频关键帧时，
                                    3.编码格式改变时会切文件时
                                    */
...
//雷达卡口数据
typedef struct tagIDM_DEV_ITS_CHECKPOINT_RADAR_INFO
{
	UCHAR ucIndex;		/*雷达序号 0-5*/
	UCHAR ucEnable;		/*是否启用 1:启用, 0不启用*/
	UCHAR ucLaneIndex;	/*车道序号 1-6*/
	UCHAR ucSnapTimes;	/*抓拍次数,1-3*/
	UCHAR ucIntervaType;	/*间隔类型：0-时间，1-距离*/
	UCHAR ucRadarType;	/*雷达类型. 0:无 1：慧昌单车道雷达 2：慧昌多车道雷达 3-森思泰克雷达*/
	UCHAR ucSerialLink;	/*关联串口 0:无 ，1～n:根据设备实际的485、232个数而定，先排485，再排232*/
	UCHAR ucIntervalNum; /* [0-6] */
	UINT uiIntervals[6];	/*连续抓拍时间间隔(单位:毫秒) 连续抓拍距离(单位:分米)*/
	CHAR uRes[128];
}IDM_DEV_ITS_CHECKPOINT_RADAR_INFO;
...
typedef struct tagIDM_DEV_ITS_CHECKPOINT_RADAR
{
	UCHAR ucRadarCount; //[0,6]
	IDM_DEV_ITS_CHECKPOINT_RADAR_INFO stRadars[6]; //
}IDM_DEV_ITS_CHECKPOINT_RADAR;

//车道事件信息
typedef struct tagIDM_DEV_LANE_EVENT
{
	UCHAR ucType;  //事件类型
	UCHAR ucEnabled; //是否开启 1开启,0不开启
	USHORT usSnapTimes; //抓拍次数
	CHAR cRes[128];
}IDM_DEV_LANE_EVENT;

//车道信息
typedef struct tagIDM_DEV_ITS_LANE_INFO
{
	UCHAR ucLaneID; //车道ID,范围[1,6]
	UCHAR ucLaneDirection; //车道方向 0:未知, 1:左转,2: 右转 ,3:左转 + 右转 ,4:直行 ,5:直行 + 左转 ,6:直行 + 右转,7:直行 + 左转 + 右转 ,8:掉头 ,9:掉头 + 直行 ,10:掉头 + 左转 
	UCHAR ucDriveDirection;//车辆行驶方向 0: 未知 ,1: 下行 ,2: 上行 ,3: 从左往右 ,4: 从右往左 ,5: 停车
	UCHAR ucLaneUsage; //车道用途 0: 未知 ,1 : 普通车道 ,2: 公交车专用道 ,3: 非机动车道 ,4: 禁止货车车道 ,5: 应急车道 ,6: 禁止调头车道
	
	UCHAR ucRadarType; //雷达类型 0:无 1：慧昌单车道雷达 2：慧昌多车道雷达 3-森思泰克雷达
	UCHAR ucRadarSerialLink;//关联串口 0:无 ，1～n:根据设备实际的485、232个数而定，先排485，再排232
	UCHAR ucLeftLineType;//左车道线类型 0: 未知 ,1: 车道间白实线 ,2: 车道间黄实线 ,3: 车道间白虚线 ,4: 车道间黄虚线 ,5: 车道停止线 ,6: 顶部车道停止线 ,7: 左边车道停止线 ,8: 右边车道停止线 ,9: 抓拍线,10: 最左车道线延伸到人行道的线，用于大弯小转 ,11: 底部礼让行人线 ,12: 顶部礼让行人线
	UCHAR ucLeftLinePointNum; //左车道线坐标数组中有效数据数量
	IDM_DEV_POINT_S stLeftLinePoints[2]; //左车道线坐标数组(坐标x,y范围[0,10000]
	
	UCHAR ucTailNumberOddLimit;//单号限行true:限行 false：不限行
	UCHAR ucTailNumberEvenLimit;//双号限行true:限行 false：不限行
	UCHAR ucRightLineType;//右车道线类型.(取值范围同左车道线)
	UCHAR ucRightLinePointNum; //右车道线点数量
	IDM_DEV_POINT_S stRightLinePoints[2];//右车道线点数组

	UCHAR ucTriggerLinePointNum;//抓拍触发线点数量
	UCHAR ucMotorVehicleEventNum;//机动车通用事件配置数量
	UCHAR ucNonMotorVehicleEventNum;//非机动车通用事件配置数量
	UCHAR ucPedestrainsEventNum;//行人通用事件配置数量
	IDM_DEV_POINT_S stTriggerLinePoints[2];//抓拍触发线点数组
	IDM_DEV_LANE_EVENT stMotorVehicleEvents[100];//机动车通用事件配置
	IDM_DEV_LANE_EVENT stNonMotorVehicleEvents[20];//非机动车通用事件配置
	IDM_DEV_LANE_EVENT stPedestrainsEvents[20];//行人通用事件配置

	UINT uiParkingTriggerTime;//违停触发时间单位秒,默认5.[5,21600]
	UINT uiParkingDurationTime;//违停持续时间单位秒,默认10. 范围[5,21600]
	USHORT usPressLineSensitivity;//压线灵敏度,默认50. 范围[1,100]
	USHORT usChangeLineSensitivity;//违法变道灵敏度,默认80 范围[1,100]
	USHORT usConversSensitivity;//逆行灵敏度,默认100, 范围.[50,300]
	USHORT usBackDriveSensitivity;//倒车灵敏度,默认100,范围[50,300]
	CHAR szComityThreshold[12]; //不礼让行人最大抓拍距离，单位是车道 100对应1车道 (浮点数字符串表示)
	CHAR szNonLocalLicenseProvince[8];//外地牌照的默认省份(一个汉字)
	
	USHORT usCarMinSpeedLimit;//小车限高速值，默认120.单位：km/h  范围[1,250]
	USHORT usCarMaxSpeedLimit;//小车限低速值，默认70,单位：km/h 范围[1,250]
	USHORT usBigMinSpeedLimit;//大车限高速值，默认120. 单位：km/h 范围[1,250]
	USHORT usBigMaxSpeedLimit;//大车限低速值，默认70 单位：km/h 范围[1,250]
	USHORT usCarSpeedLimitDeviation;//小车限速偏差值,默认10. 范围[1,250]
	USHORT usBigSpeedLimitDeviation;//大车限速偏差值,默认10. 范围[1,250]
	USHORT usCarMaxSpeed;//小车速度的最高值，单位：km/h .默认180,范围[1,250]
	USHORT usBigMaxSpeed;//大车速度的最高值，单位：km/h. 默认180,范围[1,250]
	
	USHORT usIntervalValue[8];//连拍间隔时间（单位：毫秒），连拍间隔距离（单位：分米）数量范围[1,6]
	UCHAR ucIntervalValueCount;//间隔值数量;[1,6]
	UCHAR ucIntervalType;//间隔类型：0-时间，1-距离
	UCHAR ucFlashLightMode;//闪光灯闪烁模式 0：同时闪烁 1：轮流闪烁
	UCHAR ucFlashLightCount;//爆闪灯使能数量[0,4]
	CHAR ucFlashLightEnable[4]; //爆闪灯使能
	CHAR cRes[256];
}IDM_DEV_ITS_LANE_INFO;

//卡口视频参数配置
typedef struct tagIDM_DEV_ITS_CHECKPOINT_VIDEO
{
	UCHAR ucChannelID; //通道ID. 0主机, 1从机
	UCHAR ucSnapType; //抓拍类型,默认1. 0: 机非人全部抓拍 1:仅抓拍机动车 2:仅抓拍非机动车
	UCHAR ucSceneType;//场景类型 0:普通道路 1:高速公路
	UCHAR ucSpeedType;//测速类型  0:无测速 1:视频测速 2:雷达测速 

	UCHAR ucFlashMode;//补光模式  0：频闪抓拍  1：爆闪抓拍
	UCHAR ucMaxPointNum;//区域框坐标点的最大个数,默认10
	UCHAR ucMinPointNum;//区域框坐标点的最小个数
	UCHAR ucPassDetectAreaPointCount;//过车区域检测框点数量
	IDM_DEV_POINT_S stPassDetectAreaPoints[10];//过车区域检测框点数组 最多10个

	UCHAR ucStopLinePointCount;
	UCHAR ucCrossBttomLinePointCount;
	UCHAR ucCrossTopLinePointCount;
	UCHAR ucIllegalParkPointCount;
	IDM_DEV_POINT_S stIllegalParkPoints[10];//违法停车检测框点,最多10个
	IDM_DEV_POINT_S stCrossTopLinePoints[2];//卡口违规检测人行横道顶端线
	IDM_DEV_POINT_S stCrossBttomLinePoints[2];//卡口违规检测人行横道底端线
	IDM_DEV_POINT_S stStopLinePoints[2];//卡口违规检测停止线,最多10个
	UINT uiAreaBeforeWaitTime;//违法停车检测框雷达测速,等待前时间单位毫秒,默认1000
	UINT uiAreaAfterWaitTime; //违法停车检测框雷达测速, 等待后时间, 单位毫秒,默认1000
	UINT uiLaneNum;//车道数量[1,6]
	IDM_DEV_ITS_LANE_INFO stLaneInfos[6];//车道信息
	UCHAR ucRes[256];

}IDM_DEV_ITS_CHECKPOINT_VIDEO;

//视频电警
typedef struct tagIDM_DEV_ITS_RAFFIC_LIGHT
{
	UCHAR ucLightNum;//交通灯个数 0：单个 1：三个
	UCHAR ucEnable;//交通灯使能
	UCHAR ucLightType;//交通灯放置方式 0：水平放置 1：竖直放置
	UCHAR ucColorType;//交通灯颜色 0： 红  1：绿 2：黄
	USHORT usDirection;//交通灯方向 :1：左转  16：直行 256：右转 4096：掉头
	USHORT usPointCount;
	IDM_DEV_POINT_S points[20];//交通灯位置
	UCHAR rcRes[128];
}IDM_DEV_ITS_RAFFIC_LIGHT;

//电警卡口
typedef struct tagIDM_DEV_ITS_TRAFFIC_EPOLICE
{
	UCHAR ucSnapType;//抓拍类型 0: 机非人全部抓拍 1:仅抓拍机动车 2:仅抓拍非机动车
	UCHAR ucChannelID;//通道ID 0:主机 1:从机
	UCHAR ucSceneType;//场景类型 0:普通道路 1:高速公路
	UCHAR ucSpeedType;//测速类型  0:无测速 1:视频测速 2:雷达测速 

	UCHAR ucMaxPointNum;//规则框坐标点数量最大值
	UCHAR ucMinPointNum;//规则框坐标点数量最小值
	UCHAR ucExternLightEnable;//使用外部交通灯信息
	UCHAR ucForceLightEnable;//是否使能信号灯强制转换

	USHORT usDetectRegionPointCount;//[0,10]
	USHORT usLaneNum; //车道总数[1,6]
	IDM_DEV_POINT_S stDetectReginPoints[10];//检测框坐标,最多10个点.
	IDM_DEV_ITS_LANE_INFO stLaneInfos[6];//车道信息
	IDM_DEV_POINT_S stStopLinePoints[2];//电警违规检测停止线

	IDM_DEV_POINT_S stStraightLinePoints[2];//电警违规检测直行触发线
	IDM_DEV_POINT_S stTurnLeftLinePoints[2];//电警违规检测左转边界线
	IDM_DEV_POINT_S stTurnRightLinePoints[2];//电警违规检测右转边界线
	IDM_DEV_POINT_S stCrossTopLinePoints[2];//电警违规检测人行横道顶端线
	IDM_DEV_POINT_S stCrossBttomLinePoints[2];//电警违规检测人行横道底端线
	IDM_DEV_POINT_S stSmallTurnLinePoints[2];//电警违规检测大弯小转线
	IDM_DEV_POINT_S stOppositeStopLinePoints[2];//电警违规检测对面来车停止线
	IDM_DEV_POINT_S stFrontLinePoints[2];//电警违规检测前置线
	IDM_DEV_POINT_S stCenterLinePoints[2];//电警违规检测中置线
	IDM_DEV_POINT_S stBackLinePoints[2];//电警违规检测后置线

	UINT uiRadarBeforeWaitTime;//雷达测速,等待前时间单位毫秒
	UINT uiRadarAfterWaitTime;//雷达测速,等待后时间,单位毫秒
	
	UINT usTrafficLightCount;
	IDM_DEV_ITS_RAFFIC_LIGHT stTrafficLights[12];//交通灯信息 最大12个
}IDM_DEV_ITS_TRAFFIC_EPOLICE;

//车流量统计
typedef struct tagIDM_DEV_ITS_TRAFFIC_FLOWRATE
{
	UCHAR ucTrafficFlowEnable;//车流量使能
	UCHAR ucAverageSpeedEnable;//车道平均速度使能
	UCHAR ucHeadTimeDistanceEnable;//车头时距使能
	UCHAR ucHeadSpaceDistanceEnable;//车头间距使能
	UCHAR ucLaneOccupancyTimeEnable;//车道时间占有率使能
	UCHAR ucLaneOccupancySpaceEnable;//车道空间占有率使能
	UCHAR ucQueueLengthEnable;//排队长度使能
	UCHAR ucSaturationEnable;//饱和度使能
	UINT ucStatisticInterval;//	统计间隔使能
	UINT ucJamThreshold;//拥堵排队长度
	UCHAR ucJamThresholdEnable;//拥堵评价使能
	UCHAR ucPassCountEnable;//过车统计使能
	UCHAR ucPassCountOsdEnable;//过车统计osd叠加使能开关
	UCHAR ucChannelID;//通道ID 0:主机 1:从机
	UCHAR ucAlgoType;//算法类型：15：卡口  16：电警
	CHAR cRes[127];

}IDM_DEV_ITS_TRAFFIC_FLOWRATE;

//道路事件
typedef struct tagIDM_DEV_ITS_TRAFFIC_RAOD_EVENT
{
	UCHAR ucJamEventEnable;//拥堵检测规则是否启用 1启用,0不启用
	UCHAR ucJamePictureNum;//拥堵检测规则取证图片数目,默认1,[1-1]
	UCHAR ucJamPeriod;//拥堵检测规则拥堵统计周期（分）[1-60] 默认10
	UCHAR ucJamThreshold;//拥堵检测规则拥堵阈值[1,100] 默认80

	USHORT usJamReportFilterTime;//拥堵检测规则上报过滤时间（分）[1,720],默认10
	UCHAR ucAccidentEnable;//事故检测规则是否启用 1启用,0不启用
	UCHAR ucAccidentPictureNum;//事故检测规则取证图片数目,默认1,[1-1]

	USHORT usAccidentThreshold;//事故检测规则事故车辆阈值, [1,100]默认50
	USHORT usAccidentReportFilterTime;//事故检测规则上报过滤时间（分）[1,720],默认10

	UCHAR ucMaxAreaNum;//最大取证区域数量 默认10
	UCHAR ucMinAreaNum;//最小规则框数量--不画规则框 默认0
	UCHAR ucPedestrianEnable;//行人检测规则是否启用. 1启用,0不启用
	UCHAR ucPedestrianPictureNum;//行人检测规则取证图片数目[1,1]

	USHORT usPedestrianDuration;//行人检测规则 行人持续时间(秒),[1-300] 默认10
	USHORT usPedestrianReportFilterTime;//行人检测规则上报过滤时间（分）[1-720],默认10

	UCHAR ucPedestrianRegionPointCount[12];//行人检测规则
	IDM_DEV_POINT_S stReginPoints[12][20]; //行人检测规则框

	UCHAR ucMaxPointNum;//最多多边形点数量 默认10
	UCHAR ucMinPointNum;//最少多边形点数量 默认0
	UCHAR ucAbandonEventEnable;//抛洒物检测规则是否启用. 1启用,0不启用
	UCHAR ucAbandonEventSensitivity; //抛洒物检测规则灵敏度.[1-100] 默认50
	IDM_DEV_POINT_S stAbandonAreaPoints[4];//抛洒物检测规检测区域 只有4个点
	
	
	UCHAR ucRoadBlockEventEnable;//是否启用路障事件检测
	UCHAR ucRoadBlockEventSensitivity;//路障事件检测灵敏度[1,10]
	UCHAR ucConstructionEventEnable;//是否启用施工事件检测. 1启用,0不启用
	UCHAR ucChannelID;//通道ID 0:主机 1:从机
	UCHAR ucAlgoType;//算法类型15：卡口  16：电警
	UCHAR ucRes[127];

}IDM_DEV_ITS_TRAFFIC_RAOD_EVENT;

//信号灯校正
typedef struct tagIDM_DEV_SIGNAL_LIGHT_CORRECT
{
	UCHAR ucLedID; //信号灯编号，取值范围[1,5]
	UCHAR ucEnable;//是否手动添加过信号灯，[1-已添加；0-删除]
	UCHAR ucSensitivity;//检测灵敏度，取值范围[0,100],默认50
	UCHAR ucSaturation;//饱和度，取值范围[0,100],默认50
	IDM_DEV_POINT_S stPoints[4];//信号灯位置坐标区域，一个矩形框。坐标无默认值，根据区域放大位置和倍数不同而不同。
	UCHAR uRes[128];
}IDM_DEV_SIGNAL_LIGHT_CORRECT;

typedef struct tagIDM_DEV_ITS_LED_CORRECT 
{
	UCHAR ucEnable;//是否启动信号灯校正功能：1:启动；0:关闭
	UCHAR ucRes1;
	USHORT usCount;
	IDM_DEV_SIGNAL_LIGHT_CORRECT stLedCorrect[5];//信号灯的校正参数
	UCHAR ucRes[128];
}IDM_DEV_ITS_LED_CORRECT;

/* V2版本设备树 */
typedef struct tagIDM_DEV_CHANNEL_V2
{
	CHAR      szName[128];                         /* 1.4.5.1 通道 名称	    */
	ULONG     ulUniqueChannel;					   /* 1.4.5.2 通道 唯一编号 */
	ULONG     ulRemoteChannel;					   /* 1.4.5.3 通道 远程编号 */
	UCHAR     ucIsOnline;					       /* 1.4.5.4 通道 是否在线，0：不在线，1：在线 */
	UCHAR     ucUnused[200];
}IDM_DEV_CHANNEL_S_V2;

typedef struct tagIDM_DEV_DEVICE_V2
{
	CHAR      szId[IDM_DEVICE_ID_LEN];				/* 1.4.1 设备标识   */
	CHAR      szName[IDM_DEVICE_NAME_LEN];			/* 1.4.2 设备名称	*/
	CHAR      szIPAddress[IDM_DEVICE_IP_MAX_LEN];   /* 1.4.3 设备IP     */
	CHAR      szDeviceClass[32];	                /* 1.4.4 设备大类；
													"IPC"：普通网络摄像机；
													"ITC"：智能交通摄像机；
													"NVR"：分销NVR；
													"EC"：边缘计算节点；
	...
	IDM_DEV_CHANNEL_S_V2*  pChannelList;            /* 1.4.6 该设备下的通道信息列表 */
	ULONG     ulChannelListLen;					    /*                         数量 */
	UCHAR     ucProtocol;						    /* 1.4.5 设备协议0：Onvif，1,：GB28181，2：华智私有协议，3：RTSP；4：华智私有协议主动注册 */
	UCHAR     ucUnused[200];
}IDM_DEV_DEVICE_S_V2;

typedef struct tagIDM_LOCAL_CHANNEL_V2
{
	CHAR szName[128];                                   /* 本地通道名称 */
	UINT uiUniqueChannelNo;                                   /* 唯一本地通道号 0开始 */
	UCHAR szUnuse[124];
}IDM_DEV_LOCAL_CHANNEL_V2;

typedef struct tagIDM_DEV_DEVICE_TREE_V2
{
	CHAR                       szName[128];	             /* 1.1 设备树名称 */
	ULONG                      ulChannelTotal;			 /* 1.2 通道总数   */
	ULONG                      ulChannelOnline;			 /* 1.3 通道在线数量 */
	IDM_DEV_DEVICE_S_V2      * pDevices;                 /* 1.4 下级设备树 */
	ULONG                      ulDevicesCount;           /* 1.5 下级设备数量 */
	IDM_DEV_LOCAL_CHANNEL_V2 * pstLocalChannelList;      /* 1.6 本地模拟通道列表 */
	UINT                       uiLocalChanelLisLen;      /* 1.7 本地模拟通道列表长度 */
	UCHAR     ucUnused[128];
}IDM_DEV_DEVICE_TREE_S_V2;

typedef struct tagDEV_TREE_GET_RESPONSE_V2
{
	IDM_DEV_DEVICE_TREE_S_V2   stDeviceTree;             /* 1.设备树 */
}DEVTREE_GET_RESPONSE_S_V2;

typedef struct tagIDM_DEV_TIME_PARAM_V2
{
	CHAR                       szDateTime[64];      /* 系统时间，ISO8601格式 */
	UCHAR                      unUsed[128];
}IDM_DEV_TIME_PARAM_S_V2;

/* 设备重连 */
typedef struct tagIDM_DEV_RECONNECT_INFO
{
    UINT uiInterval;  // 重连时间间隔，单位 : 毫秒(最小值3000)
    UCHAR ucEnable;   // 是否重连，0 - 不重连，1 - 重连，默认值为0
    UCHAR ucRes[123];
}IDM_DEV_RECONNECT_INFO_S;

//标定工作模式
typedef struct tagIDM_DEV_SYSMGR_RUN_MODE
{
	UCHAR ucMode; /*设备运行模式：0 工作模式；1 标定模式*/
	CHAR szUnused[127];
}IDM_DEV_SYSMGR_RUN_MODE_S;

//启动标定参数
typedef struct tagIDM_DEV_SYSMGR_CALIBRATION
{
	USHORT usWidth; /*角标之间宽度，单位：毫米，取值范围： 900- 1100*/
	USHORT usHeight; /*角标之间高度，单位：毫米，取值范围：600-800 */
	CHAR szUnused[124];
} IDM_DEV_SYSMGR_CALIBRATION_S;

//标定拼接距离
typedef struct tagIDM_DEV_SYSMGR_STITCH_DISTANCE
{
	UCHAR ucDistance; /*拼接距离，单位：米，取值范围：0~10*/
	CHAR szUnused[127];
}IDM_DEV_SYSMGR_STITCH_DISTANCE_S;


/* 强制I帧 */
typedef struct tagIDM_DEV_FORCE_I_FRAME_STREAM
{
	UINT   uiChannelNo;                   /* 通道号从0开始 0xFFFF无效值 */
	UINT   uiStreamType;                  /* 码流类型 0-主码流 1-辅码流 2-三码流 3-四码流 4-五码流 */
	UCHAR  ucUnused[52];
}IDM_DEV_FORCE_I_FRAME_STREAM;

typedef struct tagIDM_DEV_FORCE_I_FRAME
{
	UINT  uiParamType;                                       /* 参数类型 0-填入句柄 1-填入通道和码流类型 2-填入会话标识<预留 暂不支持> */
	union
	{
		UINT                            uiRealPlayHandle;    /* 参数类型0 实时预览句柄 */
		IDM_DEV_FORCE_I_FRAME_STREAM    stStreamInfo;        /* 参数类型1 填入通道号和码流类型方式 */
		UINT                            uiStreamSessionID;  /* 参数类型2 填入实况会话标识方式 */
	};
	UCHAR  ucUnused[60];
}IDM_DEV_FORCE_I_FRAME_S;


typedef struct tagIDM_DEV_PTZ_PT_ZOOM_INFO
{
	INT          iPtX;                                    /*    水平坐标，范围：[0, (360000 - 1)] */
	INT          iPtY;                                    /*	垂直坐标，范围：[-20 * 1000, 90 * 1000] */
	INT          iPtZ;                                    /*	变倍Zoom值，范围：[1*1000, 44*1000] */
	UCHAR        ucFocusFlag;                             /*	聚焦标志位[只读] 0-未聚焦 1-已聚焦 */
	UCHAR        unUsed[245];                             /*    预留 */
}IDM_DEV_PTZ_PT_ZOOM_INFO_S;

/* 辅助聚焦参数 */
typedef struct tagIDM_DEV_ASSIST_FOCUS_INFO
{
	UCHAR        ucFocusFlag;                               /* 是否聚焦标识 */
	UCHAR        unUsed[127];                               /*    预留 */
}IDM_DEV_ASSIST_FOCUS_INFO_S;
...
*@return: 成功返回IDM_SUCCESS，失败返回错误码
*@note: 发生通道离线回调时ulhandle 标识离线的通道号
*/
...
*@return: 成功返回IDM_SUCCESS，失败返回错误码
*/
IDM_API LONG IDM_DEV_SetReconnect(IDM_DEV_RECONNECT_INFO_S stReconnectInfo);
...
*@param: IN ulChannel 通道号(无需携带通道 填0xFFFF或者65535)
...
*@return: 成功返回IDM_SUCCESS，失败返回错误码
**/
IDM_API LONG IDM_DEV_GetDevTree_V2(LONG lUserID, DEVTREE_GET_RESPONSE_S_V2 *pstResponse);
...
*@return: 成功返回IDM_SUCCESS，失败返回错误码
*/
IDM_API LONG IDM_DEV_ForceSendIFrame(LONG lUserID, IDM_DEV_FORCE_I_FRAME_S *pstRequest);
...
*@return: 成功返回IDM_SUCCESS，失败返回错误码
*/
IDM_API LONG IDM_DEV_SetLocalDeviceName(LONG lUserID, CHAR *szDeviceName);
